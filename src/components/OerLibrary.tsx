...
import { Library, BookOpen, Search, Shield, Filter, ExternalLink, Bookmark } from 'lucide-react';
import { OerCorpusDoc, GradeLevel, StudentProfile } from '../types';
import { api } from '../services/api';

interface OerLibraryProps {
  currentStudent?: StudentProfile | null;
}

export const OerLibrary: React.FC<OerLibraryProps> = ({ currentStudent }) => {
  const [corpus, setCorpus] = useState<OerCorpusDoc[]>([]);
  const [selectedSubject, setSelectedSubject] = useState<string>('all');
  const [selectedGrade, setSelectedGrade] = useState<string>(
    currentStudent?.gradeLevel || 'all'
  );
  const [searchQuery, setSearchQuery] = useState('');
  const [selectedDoc, setSelectedDoc] = useState<OerCorpusDoc | null>(null);
  ...
  const filteredDocs = corpus.filter((d) => {
    const matchesSearch =
      ...
      d.keyConcepts.some((k) => k.toLowerCase().includes(searchQuery.toLowerCase()));

    const matchesGrade =
      selectedGrade === 'all' ||
      (selectedGrade === 'Grade 11-12' && d.gradeLevel === 'Grade 11-12') ||
      (selectedGrade === 'Grade 9-10' && d.gradeLevel === 'Grade 9-10') ||
      (selectedGrade === 'Grade 6-8' && d.gradeLevel === 'Grade 6-8');

    return matchesSearch && matchesGrade;
  });

  return (
    <div id="oer-library-container" className="max-w-7xl mx-auto px-4 sm:px-8 py-5">
      {/* Streamlined Header */}
