...
import { StudentProfile, ScholarshipMatchResult, GradeLevel } from '../types';
...
export const ScholarshipMatcher: React.FC<ScholarshipMatcherProps> = ({ currentStudent }) => {
  const [gradeLevel, setGradeLevel] = useState<GradeLevel>(currentStudent?.gradeLevel || 'Grade 11-12');
  const [annualIncome, setAnnualIncome] = useState<number>(140000);
  const [category, setCategory] = useState<string>(currentStudent?.category || 'OBC');
  const [gender, setGender] = useState<string>(currentStudent?.gender || 'Male');
  const [academicScore, setAcademicScore] = useState<number>(currentStudent?.academicScorePercent || 75);
  const [firstGen, setFirstGen] = useState<boolean>(currentStudent?.firstGenerationLearner || true);
  const [matches, setMatches] = useState<ScholarshipMatchResult[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [selectedScheme, setSelectedScheme] = useState<ScholarshipMatchResult | null>(null);

  // Sync with current student if changed
  useEffect(() => {
    if (currentStudent) {
  ...
  return (
    <div id="scholarship-matcher-container" className="max-w-7xl mx-auto px-4 sm:px-8 py-5">
      {/* Header */}
                  ...
                  >
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <div className="shrink-0 flex items-center gap-2">
                        {item.isEligible ? (
                          <span className="bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 font-bold text-[10px] uppercase tracking-wider flex items-center gap-1">
                            <CheckCircle2 className="w-3 h-3" />
                            Eligible
                          </span>
                        ) : (
                          <span className="bg-[#F3F4F6] text-[#6B7280] border border-[#E5E7EB] px-2 py-0.5 font-bold text-[10px] uppercase tracking-wider flex items-center gap-1">
