...
import { GroundedDoubtMessage, GradeLevel, LanguageCode, StudentProfile, OerCitation } from '../types';
...
const NCERT_SAMPLE_CATEGORIES: SampleCategory[] = [
  {
    ...
    doubts: [
      ...
      },
      {
        ...
        question: 'Why is the horizontal range of a projectile identical for complementary angles of projection θ and (90° - θ)? Show the NCERT derivation.',
...
export const DoubtSolver: React.FC<DoubtSolverProps> = ({
  ...
  const [messages, setMessages] = useState<GroundedDoubtMessage[]>([
    {
      id: 'welcome-msg',
      role: 'assistant',
...
You can ask any doubt in Physics, Chemistry, Mathematics, or Biology across Classes 6 to 12. Every solution provided is step-by-step and strictly grounded in official NCERT National Curriculum textbooks with transparent page and chapter citations.`,
  ...
  const [explanationStyle, setExplanationStyle] = useState<'step-by-step' | 'simple-analogy' | 'prerequisite-basics'>('step-by-step');
  const [imagePreview, setImagePreview] = useState<string | null>(null);
  const [isLoading, setIsLoading] = useState(false);
  const [activeCategoryIndex, setActiveCategoryIndex] = useState(0);
  const [sidebarTab, setSidebarTab] = useState<'citations' | 'practice' | 'syllabus'>('citations');
  ...
  const handleSubmit = async (overrideText?: string) => {
    ...
    const newMsg: GroundedDoubtMessage = {
      id: userMsgId,
      role: 'user',
    ...
    try {
      ...
      const assistantMsg: GroundedDoubtMessage = {
        id: `asst-${Date.now()}`,
        role: 'assistant',
      ...
      const errorMsg: GroundedDoubtMessage = {
        id: `err-${Date.now()}`,
        role: 'assistant',
  ...
  const handleSelectSample = (sample: { question: string; lang?: LanguageCode; topicId: string }) => {
    if (sample.lang) {
  ...
  // Get the latest assistant citations for the sidebar
      ...
      <div className="bg-white border border-[#E5E7EB] p-3 mb-4 flex flex-col gap-2.5">
        <div className="flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex flex-wrap gap-1.5">
            {NCERT_SAMPLE_CATEGORIES.map((cat, idx) => (
              <button
                key={idx}
                onClick={() => {
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-[#F0F2F5]">
          ...
          {NCERT_SAMPLE_CATEGORIES[activeCategoryIndex].doubts.map((s, idx) => (
            <button
              key={idx}
                  ...
                  {msg.citations && msg.citations.length > 0 && (
                    <div className="mt-3.5 pt-3 border-t border-[#E5E7EB] bg-[#F8F9FA] -mx-4 -mb-4 p-3">
                      ...
                      <div className="space-y-1.5">
                        {msg.citations.map((cite: OerCitation) => (
                          <div
                            key={cite.id}
