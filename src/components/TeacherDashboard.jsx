...
export const TeacherDashboard = ({
  ...
  const handleCreateClass = async (e) => {
    ...
    try {
      const res = await api.createClass({
        ...
        section: newSection,
        subject: newSubject,
        academicYear: newAcademicYear.trim() || "2025-2026",
  ...
  return <div id="teacher-dashboard-container" className="max-w-7xl mx-auto px-4 sm:px-8 py-5">
    ...
    /* Streamlined Header with Class Filter & Actions */
      ...
      <div className="bg-white dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 rounded-2xl shadow-xs p-5 mb-5 flex flex-wrap items-center justify-between gap-4">
        <div>
      ...
      <div className="bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800 rounded-xl p-3.5 mb-5 flex flex-wrap items-center justify-between gap-3 text-xs">
      ...
      <div className="bg-white dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 rounded-xl p-1.5 mb-4 flex gap-1 flex-wrap">
        <button
      ...
      {/* View: Classroom Resources & Study Notes */}
        ...
        <div className="space-y-4">
          <div className="bg-white dark:bg-[#18181b] border border-slate-200 dark:border-zinc-800 rounded-xl shadow-xs p-5 space-y-4">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 dark:border-zinc-800 pb-3">
              <div>
                <h3 className="font-bold text-sm text-slate-900 dark:text-white">Classroom Study Resources & Verified Uploads</h3>
                <p className="text-xs text-slate-500 dark:text-zinc-400">Class notes, PDFs, formulas, and worksheets active for [{selectedClassCode !== "all" ? selectedClassCode : (teacherClasses[0]?.classCode || "KV-10A")}]</p>
              </div>
              <div className="flex items-center gap-2">
                <button
                  type="button"
                  onClick={() => {
                    window.location.hash = "#classhub";
                  }}
                  className="clean-button-primary py-1.5 px-3 text-xs font-bold flex items-center gap-1.5"
                >
                  <BookOpen className="w-3.5 h-3.5" />
                  <span>Open Full Classroom Workspace →</span>
                </button>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
              {[
                { title: "Wave Optics & Interference Derivations", subject: "Physics", format: "PDF Document", size: "2.4 MB", verified: true, date: "Aug 2026" },
                { title: "Quadratic Equation Mastery Formula Sheet", subject: "Mathematics", format: "Formula Sheet", size: "1.1 MB", verified: true, date: "Aug 2026" },
                { title: "Thermodynamics & Heat Capacities Summary", subject: "Physics", format: "Study Notes", size: "3.2 MB", verified: true, date: "Aug 2026" },
                { title: "Chemical Kinetics Reaction Mechanisms Guide", subject: "Chemistry", format: "Diagram Notes", size: "1.8 MB", verified: true, date: "Aug 2026" }
              ].map((res, i) => (
                <div key={i} className="p-3.5 bg-slate-50 dark:bg-zinc-900/60 border border-slate-200 dark:border-zinc-800 rounded-xl space-y-2 hover:border-slate-400 dark:hover:border-zinc-600 transition-all">
                  <div className="flex items-start justify-between gap-2">
                    <span className="px-2 py-0.5 text-[10px] font-bold rounded bg-indigo-100 dark:bg-indigo-950 text-indigo-800 dark:text-indigo-200">
                      {res.subject}
                    </span>
                  ...
                  <h4 className="font-bold text-xs text-slate-900 dark:text-white line-clamp-2">{res.title}</h4>
                  <div className="flex items-center justify-between text-[10px] text-slate-400 font-mono pt-1 border-t border-slate-200 dark:border-zinc-800">
                  ...
                  <button
                    ...
                    className="w-full mt-1 py-1 px-2 text-[11px] font-bold bg-white dark:bg-zinc-800 border border-slate-300 dark:border-zinc-700 text-slate-700 dark:text-zinc-200 rounded hover:bg-slate-100 dark:hover:bg-zinc-700 transition-colors"
                  >
                    View in Classroom Workspace
                  </button>
                </div>
              ))}
            </div>
          ...
          {generatedLessonPlan && selectedTopicForPlan && (
            <div className="bg-white border-2 border-black p-5 space-y-3">
              <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
                <div>
                  <span className="text-[10px] uppercase font-bold text-[#9CA3AF]">Diagnostic Remediation Lesson Plan (Socratic Core Grounded)</span>
      ...
      {showInviteModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-[#18181b] border border-slate-300 dark:border-zinc-700 max-w-md w-full p-6 space-y-4 rounded-2xl shadow-2xl text-slate-900 dark:text-white">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
              <div className="flex items-center gap-2">
      ...
      {showCreateClassModal && (
        <div className="fixed inset-0 bg-black/40 backdrop-blur-xs flex items-center justify-center p-4 z-50">
          <div className="bg-white dark:bg-[#18181b] border border-slate-300 dark:border-zinc-700 max-w-lg w-full p-6 space-y-4 rounded-2xl shadow-2xl text-slate-900 dark:text-white">
            <div className="flex items-center justify-between border-b border-[#E5E7EB] pb-3">
              <div className="flex items-center gap-2">
                <KeyRound className="w-4 h-4 text-black" />
                <div>
